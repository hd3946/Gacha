...
const buildSetup = async () => {
  console.log('buildSetup', buildDir, NFT_BUILD_PATH);
  if (fs.existsSync(buildDir)) {
    await fs.rmdirSync(buildDir, { recursive: true });
  }
  await fs.mkdirSync(buildDir);
  await fs.mkdirSync(`${buildDir}/json`);
  await fs.mkdirSync(`${buildDir}/images`);
  if (gif.export) {
    await fs.mkdirSync(`${buildDir}/gifs`);
  }
};
...
const startCreating = async (growSize, userId) => {
  ...
  for (
    ...
    i <= growSize; //layerConfigurations[layerConfigurations.length - 1].growEditionSizeTo;
    i++
  ) {
  ...
  while (layerConfigIndex < layerConfigurations.length) {
    ...
    while (
      editionCount <= growSize //layerConfigurations[layerConfigIndex].growEditionSizeTo
    ) {
